...
def visualize(gpu, show_pc=True):
    ...
    mem_changed = np.any(mem_hist != mem_hist[0], axis=0) if mem_hist.size else np.zeros(0, dtype=bool)
    mem_nonzero = np.any(mem_hist != 0, axis=0) if mem_hist.size else np.zeros(0, dtype=bool)
...
def save_animation(gpu, out_path="tinygpu_run.gif", fps=10, max_frames=200, dpi=100):
    ...
    for i, frame_idx in enumerate(indices):
        ...
        regs = reg_hist[:frame_idx + 1].reshape(-1, gpu.num_threads * gpu.num_registers).T
